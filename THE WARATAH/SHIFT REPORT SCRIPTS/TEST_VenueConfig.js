...
function testVenueSpecificConfig() {
  ...
  if (venueName === 'WARATAH') {
    // Waratah-specific checks
    if (config.dayCount !== 5) {
      Logger.log(`❌ FAILED: Waratah should have 5 days, got ${config.dayCount}`);
      return false;
    }

    if (config.sheetNames.master !== 'MASTER ACTIONABLES SHEET') {
      Logger.log(`❌ FAILED: Waratah master sheet name incorrect`);
      return false;
    }
    ...
    Logger.log('✅ PASSED: Waratah configuration correct');
  } else if (venueName === 'SAKURA') {
