/****************************************************
 * UI SERVER — SAKURA TASK MANAGEMENT
 *
 * Bridge functions for the Task Manager React dialog.
 * Called from the Sakura Actionables Sheet GAS project.
 *
 * Dependencies:
 *   EnhancedTaskManagement_Sakura.gs - TASK_CONFIG, COLS, TOTAL_COLS,
 *                                       ETM_ACTIVE_STATUSES, createTask(), logAuditEntry_()
 ****************************************************/
...
function updateTask(rowNum, updates) {
  ...
  // Read the entire row once, update relevant indices, write back in a single call.
  // Column indices in the array are 0-based (COLS constants are already 0-based).
  ...
  // Single write: replaces all individual setValue() calls
 ...
 * Adapter: maps UI field names to canonical createTask() in EnhancedTaskManagement_Sakura.gs.
 */
