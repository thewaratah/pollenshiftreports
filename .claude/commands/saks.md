...
argument-hint: [task description for Sakura House]
---

# /saks — Sakura House Full-Pipeline Command

> **Single-command trigger for the complete Sakura House workflow:**
> Plan → Code (sakura-gas-agent) → Review (gas-code-review-agent) → Docs (documentation-agent) → Deploy (deployment-agent)
...
You are the **Sakura House Pipeline Orchestrator**. You coordinate the full Sakura House development workflow across 4 specialist agents, running independent steps in parallel and gating dependent steps sequentially. You do not write code directly.
...
1. Read `CLAUDE_SAKURA.md` to understand the area being changed
2. Classify the task:
   - **Code change** (modifies .gs files) → run all 4 phases
   - **Docs only** (no code change) → skip to Phase 3
   - **Deploy only** (code already written and reviewed) → skip to Phase 4
3. Identify which specialist agents are needed beyond `sakura-gas-agent`:
   - Rollover or trigger changes? → also dispatch `rollover-trigger-agent` in parallel with sakura-gas-agent
...
**Dispatch `sakura-gas-agent` with a focused prompt:**

```
Task: [full task description from $ARGUMENTS]
Venue: Sakura House
Read: CLAUDE_SAKURA.md first
Implement the change. Use named ranges, clearContent(), Script Properties.
Check P0/P1 rules. Do NOT clasp push — return: files changed, line numbers, what changed, why.
```

**If Phase 0 identified additional specialist agents** (rollover, Slack, task mgmt, warehouse), dispatch them IN PARALLEL with sakura-gas-agent in the same message — one Task call each.
...
```
Review Sakura House changes from this task: [task description]
Changed files: [list from Phase 1 output]
Apply P0–P3 severity rules. Return: CLEAR TO DEPLOY or list of blocking issues.
```

**If review returns blocking issues:**
- Dispatch `sakura-gas-agent` again to fix P0/P1 issues
...
```
Update docs for this Sakura House change: [task description]
Files changed: [list from Phase 1]
Update all affected paths:
- CLAUDE.md Recent Updates section
- CLAUDE_SAKURA.md (if architecture, named ranges, or Script Properties changed)
- docs/sakura/ files (if architecture, integration flows, or cell references changed)
- SAKURA HOUSE/FILE EXPLAINERS/ — update the 1–2 relevant manager handover docs:
...
```
Deploy Sakura House Shift Reports.
...
Run pre-deployment checklist, clasp push from SAKURA HOUSE/SHIFT REPORT SCRIPTS/,
post-deployment verification, then git commit + push to sakura/develop.
After clasp push succeeds, run: node scripts/sync-explainers-to-drive.js --venue sakura
```
...
```
## /saks Complete — [task description]
...
- [e.g. run menu function in spreadsheet]
```
...
- Phase 1: dispatch all implementation agents together (sakura-gas-agent + any extras)
